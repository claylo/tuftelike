...
#let instructional-extensions(theme: auto) = (
    prompt: (attrs, body) => with-theme(theme, th => block(inset: role(th, "prompt").inset, styled(th, "prompt", body))),
    response: (attrs, body) => with-theme(theme, th => block(inset: role(th, "response").inset, styled(th, "response", body))),
)
