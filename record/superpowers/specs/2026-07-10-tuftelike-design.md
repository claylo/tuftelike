...
TOC layout (ported page-against-page from the prototype's first-print PDF): PART/APPENDICES group headers at the outline margin (serif semibold, 0.16em tracking, caps); entry rows padded 2em in; chapter/appendix numbers right-aligned in their column via a two-space pad on single-char prefixes; level-2 rows one prefix column deeper (`entry.indented`, 1.5em gap), upright, 1pt smaller; no second tier under appendices or unnumbered sections; the first unnumbered entry after the numbered body (About the Author / Colophon / Index) opens the backmatter group with a 1.2em separator gap, detected structurally (numbered-predecessor query), not by title text. Folio placement is `theme.toc-pagenums`: `"ragged"` (default — folio follows the title after a fixed gap, the Tufte style) or `"flush"` (right edge). `about-author()`/`colophon()` emit outlined-but-invisible headings so they land in the contents and PDF bookmarks without altering their page typography.
